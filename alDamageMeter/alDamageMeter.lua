...
local anchor = "BOTTOM"
local x, y = pixel_scale(-647), pixel_scale(23)
local barheight = 12.35
local spacing = pixel_scale(1)
local maxbars = 10
local width, height = pixel_scale(159), pixel_scale(maxbars * (barheight + spacing) - spacing)
local maxfights = 5
local reportstrings = 5
local texture = media.files.statusbar_c
local backdrop_color = {0, 0, 0, 0}
local border_color = {0, 0, 0, 0}
local border_size = pixel_scale(2)
...
local UpdateBars = function()
    ...
    for i = 1, #barguids do
        ...
        if not bar[i] then 
            bar[i] = CreateBar()
            bar[i]:SetPoint("TOP", 0, -(config["Bar height"] + config["Bar spacing"]) * (i-1) - 12.35)
        end
        bar[i].id = i + offset
...
local OnEvent = function(self, event, ...)
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        elseif eventType=="SPELL_DISPEL" then
            if IsFriendlyUnit(sourceGUID) and combatstarted then -- and IsFriendlyUnit(destGUID)
                sourceGUID = owners[sourceGUID] or sourceGUID
    ...
    elseif event == "ADDON_LOADED" then
        ...
        if name == addon_name then
            ...
            MainFrame.title:SetPoint("TOP", MainFrame, "TOP", 0, 1)
            MainFrame.title:SetText(sMode)
    ...
    elseif event == "RAID_ROSTER_UPDATE" or event == "PARTY_MEMBERS_CHANGED" or event == "PLAYER_ENTERING_WORLD" then
        if MainFrame then
            caelPanels.SetupAddonPanel(caelPanel_DamageMeter, MainFrame)
        end
