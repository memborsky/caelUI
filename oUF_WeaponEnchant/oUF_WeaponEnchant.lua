...
local function SetIconPosition(self, icons)
    ...
    icons[3]:SetPoint(anchor, icons, anchor, col * growthx * size, row * growthy * size)
end
...
local function UpdateIcons(self)
    ...
    icon = icons[3] or CreateIcon(self, icons)
    if hasThrownEnchant then
        icon.icon:SetTexture(GetInventoryItemTexture("player", 18))

        icon.expTime = thrownExpiration
        if thrownExpiration then
            icon.expTime = now+thrownExpiration/1000

            if icons.showCD then
                icon.cd:SetCooldown(now, thrownExpiration)
            end
        end

        if icons.showCharges and thrownCharges then
            icon.count:SetText(thrownCharges)
        end
